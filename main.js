...
if ('serviceWorker' in navigator) {
    navigator.serviceWorker.register('./MOVIE/sw.js');
}
...
button.addEventListener('click', () => {
  Notification.requestPermission().then((result) => {
    if (result === 'granted') {
        ...
        document.getElementById("bell_slash").style.display = 'block';
        randomNotification();
    }
  });
});
...
function randomNotification() {
    const randomItem = Math.floor(Math.random() * notifies.length);
    const notifTitle = notifies[randomItem].name;
    const notifBody = `Genres: ${notifies[randomItem].genres}.`;
    const notifImg = `${notifies[randomItem].img}`;
    const options = {
      body: notifBody,
      icon: notifImg,
    };
